...
object GuessThePassword extends App {
  ...
  def dictionaryAttack(passPhraseLength: Int)(
                         ...
                         wordGenHeuristics: Seq[String => Seq[String]] = Seq.empty
                       ): Seq[String] = {

    val words = (
      for (word: String <- dictionary.toSet; heuristic <- wordGenHeuristics; generated <- heuristic(word)) yield generated
      ) toList
